...
const InventoryDetails = () => {
  ...
  useEffect(() => {
    fetch(`https://polar-plateau-07967.herokuapp.com/add-inventory/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setSelected(data);
      });
  ...
  const updateQuan = (add, deleted) => {
    fetch(
      `https://polar-plateau-07967.herokuapp.com/add-inventory/${id}?add=${add}&deleted=${deleted}`,
      {
        method: "PUT",
      }
    )
      ...
      <div className="w-full xl:grid grid-cols-4  xl:px-24 ">
        ...
        <div className="flex flex-col py-4 w-full  items-center  bg-white rounded-lg  shadow-md md:flex-row  hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
          ...
          <Link to='/manage-inventory'
              type="submit"
              className="bg-sky-700 py-2  px-6  text-white mt-4 rounded duration-500 hover:bg-sky-800 "
            >
         Manage inventory
            </Link>
          </div>
